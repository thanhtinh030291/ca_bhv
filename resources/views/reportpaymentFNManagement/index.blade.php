...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <form action="{!! route('reportAdmins.index') !!}" method="GET" class="form-horizontal" >
                ...
                @include('layouts.admin.partials.condition_advance', ['limit_list' => $limit_list, 'limit' => $limit, 'list' => $CPS_PAYMENTS, 'urlPost' => route('reportAdmins.index'), 'search_params' => $search_params])
            </form>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    @if (count($CPS_PAYMENTS) > 0)
                        {{ $CPS_PAYMENTS->appends($search_params)->links() }}
                    @endif
                </div>
                <div class="card-body">
                    @if (count($CPS_PAYMENTS) > 0)
                        <div class="table-responsive">
                            @include('reportpaymentFNManagement.table')
                        </div>
                        {{ $CPS_PAYMENTS->appends($search_params)->links() }}
                    @endif
                </div>
